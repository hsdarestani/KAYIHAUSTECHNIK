...
def room_planner_ai(request, project_pk):
    ...
    try:
        ...
        logger.exception("A+Bau Room Planner KI command failed")
...
def room_planner_vision(request, project_pk):
    ...
    calibration = {
        ...
        "existing_dimensions_confirmed": bool(request.POST.get("known_room_dimensions") == "1" or (measurement and measurement.status == m.RoomMeasurement.Status.CONFIRMED)),
    }
    ...
    try:
        ...
        logger.exception("A+Bau Room Planner vision analysis failed")
...
_ab_role_original_room_planner_vision = room_planner_vision
_ab_role_original_room_planner_ai = globals().get("room_planner_ai")


@login_required
@require_POST
def room_planner_vision(request, project_pk):
    organization = _ai_role_org(request)
    if not _ai_perm.can_use_room_ai(request.user):
        return JsonResponse({"ok": False, "error": "Raum-KI ist für deine Rolle nicht freigeschaltet."}, status=403)
    if not _ai_perm.project_allowed(request.user, organization, project_pk):
        return JsonResponse({"ok": False, "error": "Projekt nicht gefunden."}, status=404)
    return _ab_role_original_room_planner_vision(request, project_pk)


if _ab_role_original_room_planner_ai is not None:
    @login_required
    @require_POST
    def room_planner_ai(request, project_pk):
        organization = _ai_role_org(request)
        ...
        if not _ai_perm.project_allowed(request.user, organization, project_pk):
            return JsonResponse({"ok": False, "error": "Projekt nicht gefunden."}, status=404)
        return _ab_role_original_room_planner_ai(request, project_pk)
