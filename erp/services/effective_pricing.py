...
def _semantic_bo_matches(org, items):
    """Resolve internal A+Bau services against the imported B&O VA04 library.
...
def apply_effective_prices(org, catalog_items):
    ...
    Priority:
    1. non-zero B&O PriceItem addressed by A+Bau code or external code;
    2. CatalogItem's own non-zero sales price;
    3. deterministic semantic B&O match for internal A+Bau service codes;
