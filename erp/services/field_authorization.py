...
def _pdf_html_shell(org, title: str, body: str) -> str:
    org_name = html.escape(getattr(org, "legal_name", "") or getattr(org, "name", "A+Bau"))
    org_email = html.escape(getattr(org, "email", "") or "")
    org_phone = html.escape(getattr(org, "phone", "") or "")
    source_html = f'''<!doctype html><html lang="de"><head><meta charset="utf-8"><title>{html.escape(title)}</title><style>
