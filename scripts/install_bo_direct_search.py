...
def patch_document_views() -> None:
    ...
    if quote_new not in text:
        if quote_old not in text:
            raise RuntimeError("B&O priced quote catalog anchor changed")
        text = text.replace(quote_old, quote_new, 1)

    invoice_old_legacy = '"catalog": catalog_with_effective_prices(org, limit=500), "kind": "invoice"'
    invoice_new_legacy = '"catalog": [item for item in catalog_with_effective_prices(org, limit=500) if item.effective_sales_price > Decimal("0")], "kind": "invoice"'
    invoice_old_ab = '        "catalog": catalog_with_effective_prices(org, limit=500),\n'
    invoice_new_ab = '        "catalog": [item for item in catalog_with_effective_prices(org, limit=500) if item.effective_sales_price > Decimal("0")],\n'
    if invoice_new_legacy not in text and invoice_new_ab not in text:
        if invoice_old_legacy in text:
            text = text.replace(invoice_old_legacy, invoice_new_legacy, 1)
        elif invoice_old_ab in text:
            text = text.replace(invoice_old_ab, invoice_new_ab, 1)
        else:
            raise RuntimeError("B&O priced invoice catalog anchor changed")
    write(path, text)
...
def patch_template() -> None:
    ...
    if "data-bo-direct-search" not in text:
        if "ab-catalog-card" in text:
            anchor = '      <div class="ab-catalog-search">'
            if anchor not in text:
                raise RuntimeError("A+Bau document catalogue search anchor changed")
            replacement = '''      <div class="ab-bo-direct" data-bo-direct-search data-bo-search-url="{% url 'next-bo-price-search' %}">
        <div class="nx-kicker">B&O ORIGINALPREISE</div><h3>B&O-Position suchen</h3><p class="nx-muted">Direkt in der importierten VA04-Preisliste suchen. Nur Positionen mit echtem hinterlegtem Preis werden angezeigt.</p>
        ...
        <div data-bo-results class="ab-bo-results"></div>
      </div>
      <div class="ab-catalog-divider"><b>A+Bau-Vorlagen mit Preis</b><small>Nur bepreiste Schnellpositionen; B&O-Originalpositionen werden oben direkt gesucht.</small></div>
''' + anchor
      ...
      <section class="nx-card"><div class="nx-card-head"><div><h3>A+Bau-Vorlagen mit Preis</h3><p>Nur bereits bepreiste Schnellpositionen. Für B&O oben direkt in der Originalpreisliste suchen.</p></div></div>'''
            text = text.replace(anchor, replacement, 1)

    script = '<script src="{% static \'js/bo-direct-search.js\' %}?v=20260812-ab-1"></script>'
...
def guard() -> None:
    ...
    endpoint = read("erp/bo_direct_search_views.py")
    ...
    for needle in ("_is_field_user", "Keine Preisberechtigung"):
        if needle not in endpoint:
            raise RuntimeError(f"B&O direct-search price permission missing: {needle}")
    if views.count('effective_sales_price > Decimal("0")') < 2:
        raise RuntimeError("Unpriced shortcut positions are still shown in quote/invoice catalog")
    for needle in ("data-bo-direct-search", "data-bo-search-url", "B&O-Position suchen", "A+Bau-Vorlagen mit Preis", "bo-direct-search.js"):
...
install_tests()
guard()
print("A+Bau direct B&O search installed: Angebot/Rechnung search real priced VA04 rows; unresolved shortcuts are hidden from the main quick catalog.")
