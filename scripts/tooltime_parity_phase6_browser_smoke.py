...
if marker not in text:
    # A legacy generic settings smoke checked one `invoice_body` selector. Phase 6
    # owns the complete communication panel and performs a stricter scoped DOM
    # check below. Retire only the pre-existing selector token before adding the
    # Phase-6 block. This works whether the old selector sits on its own line or
    # inside a compact tuple/list generated by an earlier overlay.
    target = 'textarea[name="invoice_body"]'
    legacy_pos = text.find(target)
    if legacy_pos >= 0:
        quote_start = legacy_pos - 1
        quote_end = legacy_pos + len(target)
        if quote_start >= 0 and text[quote_start] in {"'", '"'} and quote_end < len(text) and text[quote_end] == text[quote_start]:
            token_start = quote_start
            token_end = quote_end + 1
            if token_end < len(text) and text[token_end] == ',':
                token_end += 1
            elif token_start > 0 and text[token_start - 1] == ',':
                token_start -= 1
            text = text[:token_start] + text[token_end:]
        else:
            # If an older generator escaped the surrounding quotes differently,
            # remove only the exact selector text and leave the tuple syntax valid.
            text = text[:legacy_pos] + text[legacy_pos + len(target):]
