...
<div class="nx-field-shell ab-v3-field" data-ab-v3-field>
  ...
  <section class="ab-v3-next-job">
    ...
    {% empty %}<div class="ab-v3-field-empty"><strong>Kein Einsatz wartet.</strong>Neue Termine erscheinen automatisch hier.</div>{% endfor %}
  </section>

  <div data-tabs>
    <nav class="ab-v3-field-tabs" aria-label="Einsatzstatus">
    ...
    <div class="nx-tab-panel ab-v3-field-panel is-active" data-tab-panel="planned">
      {% for event in planned %}<a class="ab-v3-field-job" href="{% url 'next-appointment-detail' event.pk %}"><time>{{ event.starts_at|date:'H:i' }}<small>{{ event.starts_at|date:'d.m.' }}</small></time><span class="ab-v3-field-job-copy"><strong>{{ event.title }}</strong><span>{% if event.project %}{{ event.project.customer.display_name }} · {{ event.location|default:event.project.title }}{% else %}Interner Termin{% endif %}</span></span><span class="ab-v3-field-job-state">Öffnen</span></a>{% empty %}<div class="ab-v3-field-empty"><strong>Keine geplanten Einsätze.</strong>Neue Termine erscheinen automatisch hier.</div>{% endfor %}
    </div>

    <div class="nx-tab-panel ab-v3-field-panel" data-tab-panel="overdue">
      {% for event in overdue %}<a class="ab-v3-field-job" href="{% url 'next-appointment-detail' event.pk %}"><time>{{ event.starts_at|date:'H:i' }}<small>{{ event.starts_at|date:'d.m.' }}</small></time><span class="ab-v3-field-job-copy"><strong>{{ event.title }}</strong><span>{% if event.project %}{{ event.project.customer.display_name }}{% endif %}</span></span><span class="ab-v3-field-job-state is-warn">Offen</span></a>{% empty %}<div class="ab-v3-field-empty"><strong>Alles erledigt.</strong>Keine überfällige Dokumentation.</div>{% endfor %}
    </div>

    <div class="nx-tab-panel ab-v3-field-panel" data-tab-panel="documented">
      {% for event in documented %}<a class="ab-v3-field-job" href="{% url 'next-appointment-detail' event.pk %}"><time>{{ event.starts_at|date:'H:i' }}<small>{{ event.starts_at|date:'d.m.' }}</small></time><span class="ab-v3-field-job-copy"><strong>{{ event.title }}</strong><span>{% if event.project %}{{ event.project.customer.display_name }}{% endif %}</span></span><span class="ab-v3-field-job-state is-done">✓ Dokumentiert</span></a>{% empty %}<div class="ab-v3-field-empty"><strong>Noch nichts dokumentiert.</strong>Abgeschlossene Einsätze landen automatisch hier.</div>{% endfor %}
    </div>
  </div>
...
class ABauV3FieldTests(SimpleTestCase):
    def test_field_home_is_structurally_rebuilt_and_keeps_operational_data(self):
        ...
        for marker in ("data-ab-v3-field", "Field Deck", "Nächster Einsatz", "data-tabs", 'data-tab="planned"', 'data-tab-panel="overdue"', "planned", "overdue", "documented", "next-appointment-detail", "next-time"):
...
def guard() -> None:
    ...
    for marker in ("data-ab-v3-field", "Nächster Einsatz", "data-tabs", "next-appointment-detail", "next-time"):
