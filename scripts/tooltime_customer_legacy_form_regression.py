...
# Preserve the ToolTime progressive layout but keep legacy German field wording.
...
new_loop = '''{% for field in location_form %}<div class="tt-field {% if field.name == 'access_notes' or field.name == 'street' %}tt-span-2{% endif %}"><label for="{{ field.id_for_label }}">{% if field.name == 'floor' %}Etage{% elif field.name == 'access_notes' %}Zugangshinweise{% else %}{{ field.label }}{% endif %}</label>{{ field }}{{ field.errors }}</div>{% endfor %}'''
if old_loop in template:
    template = template.replace(old_loop, new_loop, 1)
elif "field.name == 'floor'" not in template or "Zugangshinweise" not in template:
...
for needle in ("Etage", "Zugangshinweise", "Kunde konnte nicht gespeichert werden."):
    if needle not in final_template:
        raise RuntimeError(f"Customer German validation regression fix missing: {needle}")

print(f"{MARKER}: empty POST validation and German Einsatzort labels restored.")
