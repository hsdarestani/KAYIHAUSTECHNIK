...
MARKER = "A+BAU TOOLTIME PHASE 15 APPOINTMENT CUSTOMER 2026-08-21"
...
def _replace_once(text: str, old: str, new: str, label: str) -> str:
    if new in text:
        return text
    if old not in text:
        raise RuntimeError(f"Phase 15 appointment-customer anchor missing: {label}")
...
def _function_block(text: str, start_marker: str, end_markers: tuple[str, ...], label: str) -> tuple[int, int, str]:
    ...
    if start < 0:
        raise RuntimeError(f"Phase 15 function start missing: {label}")
    candidates = [text.find(marker, start + len(start_marker)) for marker in end_markers]
    candidates = [value for value in candidates if value >= 0]
    if not candidates:
        raise RuntimeError(f"Phase 15 function end missing: {label}")
    end = min(candidates)
...
def patch_model(module) -> None:
    ...
    if class_start < 0:
        raise RuntimeError("Phase 15 CalendarEvent model boundary missing")
    class_end = text.find("\n\nclass ", class_start + 1)
    if class_end < 0:
        class_end = len(text)
    ...
    if 'related_name="calendar_events"' not in block:
        ...
        if pos < 0:
            raise RuntimeError("Phase 15 CalendarEvent project field anchor missing")
        field = '''
...
def patch_create_and_edit(module) -> None:
    ...
    if helper_marker not in text:
        ...
        if create_pos < 0:
            raise RuntimeError("Phase 15 appointment_create boundary missing")
...
def patch_field_detail(module) -> None:
    ...
    old_identity = "{% if event.project %}{{ event.project.number }} · {{ event.project.customer.display_name }}{% else %}Interner Termin{% endif %}"
    new_identity = "{% if event.project %}{{ event.project.number }} · {{ event.project.customer.display_name }}{% elif event.customer %}{{ event.customer.display_name }} · Ohne Projekt{% else %}Interner Termin{% endif %}"
    if old_identity in template:
        template = template.replace(old_identity, new_identity)
    elif new_identity not in template:
        raise RuntimeError("Phase 15 direct customer detail identity anchor missing")
...
def install_tests(module) -> None:
    runtime_rel = "tests/test_tooltime_phase15_appointment_customer.py"
...
'''
    ...
    contract_rel = "tests/test_tooltime_phase15_appointment_customer_contract.py"
...
def guard(module) -> None:
    ...
    for marker in ('related_name="calendar_events"', 'customer = models.ForeignKey('):
        if marker not in models:
            raise RuntimeError(f"Phase 15 CalendarEvent customer model marker missing: {marker}")
    for marker in ('("erp", "0011_project_approval_flow")', 'name="customer"', 'to="erp.customer"'):
        if marker not in migration:
            raise RuntimeError(f"Phase 15 migration marker missing: {marker}")
    for marker in (
        ...
        if marker not in views:
            raise RuntimeError(f"Phase 15 backend marker missing: {marker}")
    if "event.customer.display_name" not in detail or "Ohne Projekt" not in detail:
        raise RuntimeError("Phase 15 customer-only appointment detail identity missing")
...
def run(module) -> None:
    ...
    guard(module)
    print(f"{MARKER}: customer-only appointments now persist a native customer relation, inherit a usable address and participate in search, filters, map, detail and edit flows without creating hidden projects.")
