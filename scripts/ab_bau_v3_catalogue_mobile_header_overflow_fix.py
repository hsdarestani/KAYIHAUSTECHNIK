...
/* A+BAU V3 QUOTES NEW MENU OVERFLOW FIX 2026-09-08
   The Neues Angebot dropdown must escape the commercial hero instead of being clipped. */
body.ab-apex .ttq-topbar{overflow:visible!important;z-index:30!important}
body.ab-apex .ttq-top-actions,
body.ab-apex .ttq-new-menu{position:relative!important;z-index:40!important}
...
(() => {
  ...
  // One capture-phase owner for template triggers prevents older finance/V3 layers
  // from opening the same chooser twice. It also closes any stale stacked modal.
  ...
  // Override the older fetch-based quick-project submitter. Production screenshots
  // showed TypeError: Failed to fetch; an explicit same-origin XHR gives deterministic
  // CSRF/session handling and useful HTTP errors instead of a generic browser failure.
...
def install_css() -> None:
    path = ROOT / CSS_REL
    if not path.exists():
        raise RuntimeError(f"Catalogue/dashboard/quote hotfix target missing: {CSS_REL}")
    ...
    if DASHBOARD_MARKER not in text:
        ...
        changed = True
    if QUOTE_EDITOR_MARKER not in text or QUOTE_MENU_MARKER not in text:
...
def bust_css_cache() -> None:
    path = ROOT / BASE_REL
    if not path.exists():
        raise RuntimeError(f"V3 cache-bust target missing: {BASE_REL}")
    text = path.read_text(encoding="utf-8")
    pattern = r'(/static/css/ab-bau-v3-finance-mobile-pdf-hotfix\.css\?v=)[^\"\']+'
    if not re.search(pattern, text):
        raise RuntimeError("Cache-bust link for final V3 CSS is missing")
...
def install_document_summary_parity() -> None:
    ...
    text = path.read_text(encoding="utf-8")
    if f'data-tooltime-summary-parity="20260908"' in text:
        return
    text = text.replace(
...
class ABauV3CatalogueMobileHeaderOverflowFixTests(SimpleTestCase):
    ...
    def test_quote_new_menu_is_not_clipped(self):
        css = (ROOT / "static/css/ab-bau-v3-finance-mobile-pdf-hotfix.css").read_text(encoding="utf-8")
...
def guard() -> None:
    ...
    for required in (EDITOR_RUNTIME_MARKER, "event.stopImmediatePropagation()", "new XMLHttpRequest()", "MutationObserver"):
...
def main() -> None:
    ...
    print(f"{QUOTE_MENU_MARKER}: Neues Angebot dropdown can render fully above following content.")
    print(f"{QUOTE_EDITOR_MARKER}: project creation, template modal, position model and calculation summary hardened.")
